...
# technologies undbc.Collapseder different licensing terms should contact us at
...
class RemoteDialog:
    ...
    def stylize_page(self):
        ...
        self.copy_key = Kbutton(name=[Kritter.icon("copy"), "Copy key"], spinner=True)
        localhost.append(self.copy_key)
        ...
        self.domain_cont = dbc.Collapse(domain_card, id=self.kapp.new_id(), is_open=self.config['subdomain']!="", style=style)
    ...
    def setup_callbacks(self):
        ...
        script = """
            function(click, url) {
                ...
                textArea.value = url;
                textArea.style.position = "fixed";  s
        ...
        self.kapp.clientside_callback(script, Output("_none", Kritter.new_id()), [Input(self.copy_key.id, "n_clicks")], state=[State(self.key_store.id, "data")])


    def text_visor_commands(self):
        def remote(words, sender, context):
            # start or get thread for remote tunnel
            start()
            status = 'no address yet' if self.address=='' else f"Go to [vizy link]({self.address}), Created at {datetime.datetime.now().strftime('%I:%M:%S %p')}"
            return status
        ...
        def thread():
            ...
            # ssh from asking if you want to connect to an "unknown host" (unknown to it).  
            # todo: make ternary
            if self.config['subdomain']!="": command = ["ssh", "-i", self.key_filename, "-oStrictHostKeyChecking=no", "-R", f"{self.config['subdomain']}.{self.config['domain']}:80:localhost:80", "root@localhost.run", "--", "--output", "json"]
            else: command = ["ssh", "-oStrictHostKeyChecking=no", "-R", "80:localhost:80", "nokey@localhost.run", "--", "--output", "json"]
            while self.run:
                self.process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
                while True:
                    out = self.process.stdout.readline()
        ...
        # build and append commands to text visor
        tv_table = KtextVisorTable({"remote": (remote, "Get SSH tunnel link to Vizy")})
